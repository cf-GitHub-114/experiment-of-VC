...
BEGIN_MESSAGE_MAP(Cexp92Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_pictureButton, &Cexp92Dlg::OnBnClickedpicturebutton)
END_MESSAGE_MAP()
...
void Cexp92Dlg::OnBnClickedpicturebutton()
{
	// TODO: 在此添加控件通知处理程序代码
	fstream ifs(_T("C:\\Users\\吃饭\\Pictures\\Camera Roll\\abc.txt"));
	string s;
...
void Cexp92Dlg::laaa(CImage & img, int & x, int & y, int & w, int & h)
{
	CRect rect;
	GetDlgItem(IDC_STATIC)->GetClientRect(&rect);            //获取客户区
	float rect_ratio = 1.0*rect.Width() / rect.Height();     //计算客户区的宽高比
